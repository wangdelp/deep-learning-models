...
# replace model with cifar model ++++++
#model = ResNet50(input_tensor=gen, weights='imagenet')
model = load_model("./cifar10.mdl")

#imgs = Input(input_shape=(32, 32, 3))
#imgs = Input(tensor=gen)
#preds = model.predict(imgs)
#model = Model(imgs, preds)
...
#loss_pred = -tf.reduce_sum(y*tf.log(tf.clip_by_value(preds, 1e-10, 1.0)))
...
with sess.as_default():
  ...
  for i in range(10000):
    z_, y_ = sampling(sess, z_dim=z_dim, num_category=10, bs=batch_size, random=True)
